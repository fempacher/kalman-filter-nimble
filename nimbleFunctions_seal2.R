...
nimbleMod_seal2_priorsOnly <- nimbleCode({
  # Prior for max pup survival
  phi_p ~ dbeta(2.87, 1.78)
  # Prior for adult survival
  # phi_a0 ~ dbeta(1.6,  1.2)
...
nimbleMod_seal2_bkf <- nimbleCode({
  ...
  # mean_x[1, 1:2] <- x_upd[1, 1:2]
  for(t in 1:totalT){
    # Calculate matrices. Here's where the cheating happens because of BH.
    real_phi_p[t] <- phi_p/(1+(beta*x_upd[t, 1])^rho)
    T21[t] <- 0.5*real_phi_p[t]
    T11[t] <-  alpha*0.5*real_phi_p[t]
    # Q_matrix[,] <- diag(c(alpha*(1-alpha)*(0.5*real_phi_p*x_upd[1]+phi_a*x_upd[2]),
    #                    0.5*real_phi_p*(1-0.5*real_phi_p)*x_upd[1] + phi_a*(1-phi_a)*x_upd[2]))
    #Z_matrix <- c(1,0)
    
    # Prediction
    # x_pred[1:2] <- T_matrix%*%x_upd
    x_pred[t, 1] <- T11[t]*x_upd[t, 1] + T12*x_upd[t, 2]
    x_pred[t, 2] <- T21[t]*x_upd[t, 1] + phi_a*x_upd[t, 2]
    
    # P_pred[,] <- T_matrix%*%P_upd%*%t(T_matrix) + Q_matrix
    ...
    #Save results
    # mean_x[t+1, 1:2] <- x_upd[t+1, 1:2]
  }
})

nimbleMod_seal2_indEst <- nimbleCode({
  # Priors and constraints
  ###################################
  # Prior for initial population size
  # make init.pups/pups_survived 3rd coordinate
  ...
  for (t in 2:(totalT+1)) {
    ...
    negkappa0[t-1] ~ dgammashift(shape = kappa1, rate = 1/kappa2[t-1], shift = - x[t,2])
  }
})

nimbleMod_seal2_randEff_indEst <- nimbleCode({
  # Priors and constraints
  ###################################
  # Prior for initial population size
  # make init.pups/pups_survived 3rd coordinate
  ...
  # make init.adults.nokids/adults_survived 4th coordinate
  #init.adults.nokids ~ dnegbin(alpha, x[1,1])
  # make alpha 5th coordinate. necessary for PF in nimble
  # !!!!!! alpha[t] = x[t,5]
  # alpha[1] ~ dbeta(a, b)
  x[1, 5] ~ dbeta(a, b)
  mean_alpha <- a/(a+b)
  #x[1,4] ~ dnegbin(alpha[1], x[1,1])
  x[1,4] ~ dnegbin(mean_alpha, x[1,1])
  x[1,2] <- x[1,4] + x[1,1]
  ###################################
  # Prior for max pup survival
  phi_p ~ dbeta(2.87, 1.78)
  ...
  rho ~ dgamma(4, 1/2.5)
  # convert chi to beta
  #beta[1] <- 1/chi*((alpha[1]*0.5*phi_p)/(1-phi_a)-1)^(1/rho)
  beta <- 1/chi*((mean_alpha*0.5*phi_p)/(1-phi_a)-1)^(1/rho)
  # Prior for precision
  tau ~ dgamma(2.1, 1/66.67)
  # Likelihood
  ###################################
  ...
  for (t in 1:(totalT)){
    # alpha[t+1] ~ dbeta(a, b)
    x[t+1, 5] ~ dbeta(a, b)
    #beta[t+1] <- 1/chi*((alpha[t+1]*0.5*phi_p)/(1-phi_a)-1)^(1/rho)
    real_phi_p[t] <- phi_p/(1+(beta*x[t,1])^rho)
    # survived pups is 3rd coordinate, survived adults is 4th coordinate
    # survived_pups[t] ~ dbin(0.5*real_phi_p[t], x[t,1])
    # survived_adults[t] ~ dbin(phi_a, x[t,2])
    ...
    x[t+1,4] ~ dbin(phi_a, x[t,2])
    x[t+1,2] <- x[t+1,3] + x[t+1,4]
    # x[t+1,1] ~ dbin(alpha[t+1], x[t+1,2])
    x[t+1,1] ~ dbin(x[t+1,5], x[t+1,2])
    }
  ###################################
  # Observation process
  for (t in 2:(totalT+1)) {
